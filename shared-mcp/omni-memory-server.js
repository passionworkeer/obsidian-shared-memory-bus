...
function readEmbeddingsSummary() {
  if (!fs.existsSync(EMBEDDINGS_INDEX_PATH)) {
    return {
      exists: false, path: EMBEDDINGS_INDEX_PATH, count: 0,
      tools: {}, backends: {}, models: {}, dimensions: {}, providerHosts: {}, configHashes: {},
    };
  }
  const tools = {}, backends = {}, models = {}, dimensions = {}, providerHosts = {}, configHashes = {};
  let count = 0;
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (dim > 0) dimensions[String(dim)] = (dimensions[String(dim)] || 0) + 1;
      if (record.providerHost) providerHosts[record.providerHost] = (providerHosts[record.providerHost] || 0) + 1;
      if (record.configHash) configHashes[record.configHash] = (configHashes[record.configHash] || 0) + 1;
    } catch (err) {
  ...
  return { exists: true, path: EMBEDDINGS_INDEX_PATH, count, bytes: stat.size, updatedAt: stat.mtime.toISOString(), tools: tools, backends, models, dimensions, providerHosts, configHashes };
}
...
function buildEmbeddingIndexState(runtimeSummary, embeddingsSummary) {
  ...
  if (!embeddingsSummary?.exists || !embeddingsSummary?.count) {
    return { status: "missing", rebuildRequired: false, reason: "missing-embeddings-index", activeConfigHash, indexedConfigHash: "", indexedConfigHashes };
  }
  if (uniqueConfigHashes.length === 0) {
    return { status: "legacy", rebuildRequired: false, reason: "index-missing-config-hash", activeConfigHash, indexedConfigHash: "", indexedConfigHashes };
  }
  if (uniqueConfigHashes.length > 1) {
    return { status: "mixed", rebuildRequired: true, reason: "embeddings-index-has-mixed-config-hashes", activeConfigHash, indexedConfigHash: "", indexedConfigHashes };
  }
  const indexedConfigHash = uniqueConfigHashes[0];
  if (indexedConfigHash !== activeConfigHash) {
    return { status: "stale", rebuildRequired: true, reason: "active-runtime-differs-from-index", activeConfigHash, indexedConfigHash, indexedConfigHashes };
  }
  return { status: "aligned", rebuildRequired: false, reason: "", activeConfigHash, indexedConfigHash, indexedConfigHashes };
}
...
function annotateEmbeddingRuntimeCatalog(catalog, embeddingsSummary) {
  ...
  const annotateEntry = (entry = {}) => {
    ...
    const configHash = buildEmbeddingConfigHash({ backend: adapter, modelName, baseUrl: String(entry.baseUrl || "") });
    const indexedCount = Number(indexedConfigHashes[configHash] || 0);
    return { ...entry, configHash, indexedCount, indexCompatible: indexedCount > 0, rebuildRequired: indexedCount === 0 };
  };
  if (annotated.runtime && typeof annotated.runtime === "object") annotated.runtime = annotateEntry(annotated.runtime);
  if (Array.isArray(annotated.providers)) annotated.providers = annotated.providers.map(annotateEntry);
  if (Array.isArray(annotated.profiles)) annotated.profiles = annotated.profiles.map(annotateEntry);
  return annotated;
}

function readEmbeddingRuntimeCatalog() {
  return buildEmbeddingRuntimeCatalog({ rootPath: AI_MEMORY_ROOT, getEnvValue: firstNonEmptyEnv, defaults: EMBEDDING_RUNTIME_DEFAULTS });
}
...
function resetSearchWorkerState(reason = "") {
  searchWorkerBuffer = "";
  if (reason) searchWorkerLastError = reason;
...
function handleSearchWorkerStdout(chunk) {
  ...
  for (const line of lines) {
    ...
    if (!requestId || !searchWorkerPending.has(requestId)) continue;
...
function checkSearchWorkerCircuit() {
  ...
  if (searchWorkerCircuitOpen) {
    if (searchWorkerRestartCount < SEARCH_WORKER_MAX_RESTARTS || now - searchWorkerFirstFailureAt >= SEARCH_WORKER_CIRCUIT_WINDOW_MS) {
...
function handleSearchWorkerExit(code, signal) {
  ...
  if (checkSearchWorkerCircuit()) {
    ...
    console.error(`[omni-memory] FATAL: Search worker circuit breaker open after ${searchWorkerRestartCount} failures, manual restart required`);
    resetSearchWorkerState(reason);
    return;
  }
  searchWorkerRestartCount += 1;
  METRICS.search_worker_restarts_total = searchWorkerRestartCount;
  if (searchWorkerRestartCount === 1) searchWorkerFirstFailureAt = now;
...
async function stopSearchWorker(reason = "search-worker-stop-requested") {
  ...
  if (!child || child.killed || child.exitCode !== null) {
    return { ok: true, stopped: false, previousPid: child?.pid || null, reason: "search-worker-not-running" };
  }
  return await new Promise((resolve) => {
    ...
    const exitHandler = (code, signal) => {
      ...
      finish({ stopped: true, exitCode: code ?? null, signal: signal ?? null, reason: `search-worker-exited: code=${code ?? "null"} signal=${signal ?? "null"}` });
    };
    const timeout = setTimeout(() => {
      child.removeListener("exit", exitHandler);
      try { child.kill("SIGKILL"); } catch (_error) { /* best-effort */ }
      resetSearchWorkerState(`${reason}-timeout`);
      finish({ stopped: false, exitCode: child.exitCode ?? null, signal: "timeout", reason: `${reason}-timeout` });
    }, 5000);
    ...
    try {
      ...
      finish({ stopped: false, exitCode: child.exitCode ?? null, signal: "kill-error", reason: String(error?.message || error) });
    }
  });
}
...
async function restartSearchWorker(reason = "search-worker-restart-requested") {
  ...
  return {
    ...
    stop, before, after, health,
  };
}

async function ensureSearchWorker() {
  if (isSearchWorkerRunning()) return searchWorker;
  if (searchWorkerStartupPromise) return searchWorkerStartupPromise;
  if (!fs.existsSync(SEARCH_SCRIPT)) throw new Error(`search-script-missing: ${SEARCH_SCRIPT}`);
  if (!PYTHON.available) throw new Error(`python-runtime-unavailable: ${PYTHON.error || "unknown-error"}`);
  ...
  searchWorkerStartupPromise = new Promise((resolve, reject) => {
    const child = spawn(PYTHON.command, withPythonArgs(PYTHON, [SEARCH_SCRIPT, "--server"]), {
      ...
      windowsHide: true,
      env: { ...PYTHON_SPAWN_ENV, AI_MEMORY_OBSIDIAN_VAULT: VAULT_ROOT },
    });
    let settled = false;
    const settleResolve = () => {
      if (settled) return;
      ...
      child.on("error", (error) => { searchWorkerLastError = String(error?.message || error); });
      resolve(child);
    };
    ...
    child.once("exit", (code, signal) => {
      if (!settled) settleReject(new Error(`search-worker-startup-exited: code=${code ?? "null"} signal=${signal ?? "null"}`));
    });
  }).finally(() => { searchWorkerStartupPromise = null; });
...
async function requestSearchWorker(payload, timeoutMs = 120000) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    searchWorkerPending.set(requestId, {
      resolve: (val) => {
        if (searchWorkerRestartCount > 0) resetSearchWorkerHealth();
        resolve(val);
      },
      reject,
      timeout,
...
for (const [name, handler] of Object.entries(status.handlers)) ALL_HANDLERS[name] = handler;
for (const [name, handler] of Object.entries(retrieval.handlers)) ALL_HANDLERS[name] = handler;
for (const [name, handler] of Object.entries(generation.handlers)) ALL_HANDLERS[name] = handler;
for (const [name, handler] of Object.entries(bridge.handlers)) ALL_HANDLERS[name] = handler;
for (const [name, handler] of Object.entries(embeddings.handlers)) ALL_HANDLERS[name] = handler;
...
process.on("exit", () => {
  try {
    if (searchWorker && !searchWorker.killed) searchWorker.kill();
  } catch (_error) {
