...
if [ "$IS_WINDOWS_WRAPPER" -eq 1 ]; then
  ...
  exec "$PWSH_BIN" -NoProfile -ExecutionPolicy Bypass -File "$SCRIPT_DIR/start-default-shared-mcp.ps1" "$@"
fi
...
exec "$PWSH_BIN" -NoProfile -File "$SCRIPT_DIR/start-default-shared-mcp.ps1" "$@"
