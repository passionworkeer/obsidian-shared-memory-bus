 ...
 * All new code should use bus/store-root.js instead.
 *
 * This file now re-exports from store-root.js if available,
 * so all existing code that imports from vault-root.js
 * automatically gets the new store-root resolution.
 */
"use strict";
...
// If store-root.js exists, delegate to it for backward compat
function loadStoreRootDelegate() {
  const candidates = [
  ...
  return null; // store-root not available — fall through to legacy impl
}

const _storeDelegate = loadStoreRootDelegate();
...
function resolveVaultRoot(options = {}) {
  // Delegate to store-root.js if available (preferred path)
  if (_storeDelegate) {
    try {
      return _storeDelegate.resolveStoreRoot(options);
    } catch {
