...
if [ "$IS_WINDOWS_WRAPPER" -eq 1 ]; then
  ...
  exec "$PWSH_BIN" -NoProfile -ExecutionPolicy Bypass -File "$SCRIPT_DIR/install-client-integrations.ps1" "$@"
fi
...
exec "$PWSH_BIN" -NoProfile -File "$SCRIPT_DIR/install-client-integrations.ps1" "$@"
