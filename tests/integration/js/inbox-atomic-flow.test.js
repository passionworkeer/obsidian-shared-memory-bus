...
describe("inbox atomic flow integration", () => {
  ...
  // -------------------------------------------------------------------------
  const concurrentTest = test.name !== "concurrent 20 child processes each append one line — zero dropped lines" ||
    (!process.env.GITHUB_ACTIONS && process.platform !== "win32") ? test : test.skip;
