...
- runtime client probes now verify a real reply plus a real shared-memory write/read round-trip from Codex, Claude Code, and OpenCode by comparing returned `watchdog.pid` / `memoryIntegrity.status` against the live shared `memory_status` snapshot and requiring `insert_claude_mem` plus `query_claude_mem` success
...
- `memory_status` now refreshes embedding metrics from the on-disk index summary, so `metrics.embeddings_index.age_seconds` and `metrics.embeddings_index.size` stay accurate even when the search worker health payload does not carry embedding stats
- `verify-client-integrations.ps1` now treats dense retrieval as a first-class health gate: it requires `memory_status.embeddingIndexState.status=aligned`, a direct dense `search_shared_memory` smoke result, and client runtime probes that observe both dense recall and aligned embedding state
- `run-pressure-test.ps1` now includes `list_embedding_runtimes` plus dense `search_shared_memory` tool-call waves, and client probe waves now require both `embeddingIndexAligned=true` and `denseObservedResult=true`
...
- live installed-runtime validation on April 9, 2026 passed `verify-client-integrations.ps1 -RunCliChecks -RunRuntimeChecks` after the runtime probes were hardened to require a real natural-language reply plus `insert_claude_mem`/`query_claude_mem` success; Codex and Claude Code returned a reply, `saveOk=true`, `queryFound=true`, and matched the same live watchdog and memory-integrity state, while OpenCode remained a non-blocking auth skip on this workstation
- live installed-runtime validation on April 9, 2026 also passed `run-pressure-test.ps1 -Waves 1 -RunCliChecks -RunToolCalls -RunClientTaskChecks` with `overallPass=true`, single listeners on `9331-9338`, and stable shared PIDs during mixed control-plane, tool-call, and client-task load
- live embedding-runtime validation on April 9, 2026 confirmed the current runtime stayed aligned on this workstation: `memory_status.embeddingIndexState.status=aligned`, `list_embedding_runtimes` reported `rebuildRequired=false` for the active runtime, dense `search_shared_memory(mode='dense')` returned results, and `metrics.embeddings_index.size` no longer stayed stuck at `0`
